...
- [ ] `(tabs)/calendar.tsx` — 월 뷰 + "월/일정" 세그먼트 토글
- [ ] 이벤트 색상 규칙 — 날짜 셀 하단 dot 마커: 응모(파랑) / 이용(주황) / 지급(초록) / 해지(회색) / 경고(빨강)
- [ ] 선택 날짜 하단 해당일 이벤트 인라인 리스트 (바텀시트 → 인라인 섹션으로 변경)
- [ ] 이벤트 항목 탭 → `events/[id]` 상세 진입
